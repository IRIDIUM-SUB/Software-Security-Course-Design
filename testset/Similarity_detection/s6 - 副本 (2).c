...
statvs GetElem(SqList L,int i,int* e)
{
	if(L.listsize==0)//线性表不存在时
    return ERROR;
    if(L.listsize!=0)
...
statvs LocateElem(SqList L,int e)
{
    int i;
    if(L.listsize==0)//线性表不存在时
    return ERROR;
...
statvs PriorElem(SqList L,int cur,int* pre_e)
{
	int j;
	if(L.listsize==0)//线性表不存在时
    return ERROR;
...
statvs NextElem(SqList L,int cur,int* next_e)
{
	int j;
	if(L.listsize==0)//线性表不存在时
    return ERROR;
...
statvs ListInsert(SqList * L,int i,int e)
{   if(L->listsize==0)//线性表不存在时
    return ERROR;
    if(L->listsize!=0)
    {
...
statvs ListDelete(SqList * L,int i,int* e)
{   if(L->listsize==0)//线性表不存在时
    return ERROR;
    if(L->listsize!=0)
    {
